...
const compareFilterDeltas = require('brain.js/dist/layer/convolution').compareFilterDeltas;
...
describe('layer.Convolution.compareFilterDeltas()', () => {
  ...
  describe('algorithm shape', () => {
    ...
    function getBrainConvolutionLayerCompareFilterDeltas(settings) {
      const target = 'sum += inputs[this.thread.z][inputY][inputX] * deltas[this.constants.deltaZ][deltaY][deltaX];';
      ...
      const output = [];
      for (let i = 0; i < settings.depth; i++) {
        output.push(gpuMock(compareFilterDeltasInjected, {
  ...
  describe('output', () => {
    ...
    function setupOutputs(settings) {
      ...
      const compareFilterDeltas = [];
      for (let i = 0; i < settings.depth; i++) {
        compareFilterDeltas.push(setupCompareFilterDeltas(settings, i));
      }
      convnet.ConvLayer.prototype.backward.call(convnetInstance);
      const expected = convnetInstance.filters.map(utils.volDWToArrays);
      const result = [];
      for (let i = 0; i < settings.depth; i++) {
        result.push(compareFilterDeltas[i](filterDeltas, inputs, deltas));
      }
      return {
        expected, result
      }
